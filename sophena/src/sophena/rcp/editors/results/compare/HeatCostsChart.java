...
class HeatCostsChart {
	...
	void create(Composite body, FormToolkit tk) {
		...
		for (int i = 0; i < comparison.projects.length; i++) {
			...
			max = Math.max(max, pr.costResult.brutto.heatGenerationCosts * 1000);
			max = Math.max(max, pr.costResultFunding.brutto.heatGenerationCosts * 1000);
		}
	...
	private void addNormalTrace(int i) {
		double x = i * 30 + 10;
		double y = comparison.results[i].costResult.brutto.heatGenerationCosts * 1000;
	...
	private void addFundingTrace(int i) {
		double x = i * 30 + 20;
		double y = comparison.results[i].costResultFunding.brutto.heatGenerationCosts * 1000;
	...
	private Trace createTrace(String label, double x, double y) {
		...
		Trace trace = new Trace(label, graph.primaryXAxis, graph.primaryYAxis,
				data);
