...
public record NetworkTree(Junction root) {
	public static Res<NetworkTree> parse(JsonObject obj) {
		if (obj == null) return Res.error("Network object is empty");

		var array = Json.getArray(obj, "nodes");
		if (array == null || array.isEmpty()) return Res.error(
			"Network nodes array is empty"
		);
		...
		if (nodes.isEmpty()) return Res.error(
			"No valid nodes found in network array"
		);

		long rootId = Json.getLong(obj, "root", -1);
		if (rootId == -1) return Res.error(
			"No root node ID found in network object"
		);
	...
	private static class ParseState {
		...
		Res<Node> parse(long id) {
			...
			if (visiting.contains(id)) return Res.error(
